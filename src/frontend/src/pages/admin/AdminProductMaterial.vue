...
const API_URL = 'http://localhost:8081/admin/product-material'
...
const oldKeys = ref({ idMaterial: 0, idProduct: 0 })
...
const loadData = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Lỗi', detail: 'Không thể tải danh sách Product Material', life: 3000 })
  } finally {
    loading.value = false
  }
}
...
const loadBaseOptions = async () => {
  try {
    ...
    const materialData = Array.isArray(mRes.data) ? mRes.data : (mRes.data.content || [])
    const productData = Array.isArray(pRes.data) ? pRes.data : (pRes.data.content || [])
    materialOptions.value = materialData.map((m: any) => ({ label: m.nameMaterial, value: m.id }))
...
// Add / Edit
const openAdd = () => {
  currentItem.value = { idMaterial: null, idProduct: null }
  isEdit.value = false
  displayDialog.value = true
}

const openEdit = (item: any) => {
  currentItem.value = { idMaterial: item.idMaterial, idProduct: item.idProduct }
  oldKeys.value = { idMaterial: item.idMaterial, idProduct: item.idProduct }
  isEdit.value = true
  displayDialog.value = true
}

const saveItem = async () => {
  if (!currentItem.value.idMaterial || !currentItem.value.idProduct) {
    toast.add({ severity: 'warn', summary: 'Cảnh báo', detail: 'Vui lòng chọn đầy đủ nguyên liệu và sản phẩm', life: 2000 })
    return
  }
  ...
  try {
    if (isEdit.value) {
      await axios.put(`${API_URL}/update`, currentItem.value, {
        params: {
          oldIdMaterial: oldKeys.value.idMaterial,
          oldIdProduct: oldKeys.value.idProduct
        }
      })
      toast.add({ severity: 'success', summary: 'Thành công', detail: 'Đã cập nhật Product Material', life: 2000 })
    } else {
      await axios.post(`${API_URL}/add`, currentItem.value)
      toast.add({ severity: 'success', summary: 'Thành công', detail: 'Đã thêm mới Product Material', life: 2000 })
    }
...
// Delete
const deleteItem = async (idMaterial: number, idProduct: number) => {
  if (!confirm('Bạn có chắc muốn xóa mục này?')) return
  try {
    await axios.delete(`${API_URL}/delete`, { params: { idMaterial, idProduct } })
    toast.add({ severity: 'success', summary: 'Thành công', detail: 'Đã xóa Product Material', life: 2000 })
    loadData()
  } catch (err) {
...
const exportExcel = async () => {
  try {
    ...
    link.href = url
    link.setAttribute('download', 'product_materials.xlsx')
  ...
  <div class="pc-page">
    ...
    <div class="pc-header">
      <div class="pc-header-left">
        <i class="fas fa-industry pc-icon"></i>
        <h2>Quản lý Product Material</h2>
      </div>
    ...
    <div class="pc-toolbar">
      ...
      <div class="pc-toolbar-right">
        <div class="pc-search-box">
          ...
          <input type="text" v-model="search" placeholder="Tìm theo tên nguyên liệu, tên sản phẩm...">
        </div>
      </div>
    </div>
    ...
    <div class="pc-table-wrapper">
      <table class="pc-table">
        ...
        <tbody>
          ...
          <tr v-for="(item, index) in items" :key="item.idMaterial + '-' + item.idProduct" class="pc-row">
            ...
            <td class="text-center">
              <div class="pc-actions">
                ...
                <button class="pc-action-btn pc-action-delete" title="Xóa"
                        @click="deleteItem(item.idMaterial, item.idProduct)">
    ...
    <!-- Dialog Thêm/Sửa -->
    <Dialog v-model:visible="displayDialog" :header="isEdit ? '✏️ Chỉnh sửa Product Material' : '➕ Thêm mới Product Material'" 
      ...
      <div class="pc-form">
        ...
        <div class="pc-field">
          <label class="pc-label">Nguyên liệu</label>
          <Dropdown v-model="currentItem.idMaterial" :options="materialOptions" optionLabel="label" optionValue="value" 
                    placeholder="Chọn nguyên liệu" filter class="pc-dropdown" />
        </div>
      </div>
      <template #footer>
    ...
    <Dialog v-model:visible="visibleImportDialog" modal class="pc-import-dialog" :style="{ width: '800px' }" header="📥 Nhập liên kết Sản phẩm - Nguyên liệu">
        <div class="pc-import-review">
            <div class="pc-info-bar">
                <i class="fas fa-info-circle"></i>
                <span>Xem trước <b>{{ importPreviewList.length }}</b> sản phẩm nguyên liệu sẽ được nhập. Hệ thống sẽ tự động bỏ qua màu đã tồn tại.</span>
            </div>
